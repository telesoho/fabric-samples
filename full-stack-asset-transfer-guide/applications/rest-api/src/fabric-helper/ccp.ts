...
export function getPEM(credentials: Credentials): string {
  ...
  if (credentials.path && existsSync(credentials.path)) {
    // cert value is in a file
    const data = readFileSync(credentials.path);
    const result = Buffer.from(data).toString();
    return Utils.normalizeX509(result);
  }
  logger.error(`Credentials path ${credentials.path} does not exist`);
  return '';
}
...
export class CommonConnectionProfileHelper {
  ...
  constructor(profilePath: string, json: boolean) {
    if (json) {
      ...
      this.profile = require(configPath);
      this._makeJsonPathsAbsolute(this.profile, path.dirname(configPath));
    } else {
  ...
  public _makeJsonPathsAbsolute(parent: any, rootPath: string): void {
    if (parent && typeof parent === 'object') {
      Object.entries(parent as Record<string, unknown>).forEach(
  ...
  public getOrganizationsForChannel(channelName: string): Array<string> {
    const channelPeers = Object.keys(
      this.profile.channels[channelName].peers as Record<string, unknown>
    );
    ...
    for (const [orgName, org] of Object.entries<any>(orgs)) {
      ...
      if (
        orgPeers.filter((peerName: string) => channelPeers.includes(peerName))
          .length > 0
      ) {
